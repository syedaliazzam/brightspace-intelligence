    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {items.map((item, index) => (
        ...
        >
          ...
          <p className="text-xs font-semibold uppercase tracking-[0.22em] opacity-75 text-[#0D5C48]">{item.label}</p>
          <p className="mt-4 text-3xl font-semibold tracking-tight text-[#063F32]">{item.value}</p>
          {item.helper ? <p className="mt-2 text-sm opacity-75 text-[#245C4F]">{item.helper}</p> : null}
        </motion.article>
      ))}
    </section>
  );
