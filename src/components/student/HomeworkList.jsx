...
export default function HomeworkList({ items = [], onRefresh }) {
  ...
  return (
    <>
      <div className="overflow-x-auto rounded-[1.5rem] border border-[#2D8A6A]/15 bg-white/95 shadow-[0_18px_60px_-36px_rgba(13,59,46,0.18)]">
        <table className="min-w-full divide-y divide-[#F1EADC] text-left text-sm">
          ...
          <tbody className="divide-y divide-[#F1EADC] bg-white">
            {items.map((item) => (
              <tr key={item.id} className="align-top">
                <td className="px-4 py-4 font-semibold text-[#063F32]">
                  {item.title || "Homework"}
                  <p className="mt-1 max-w-lg font-normal text-[#245C4F]">{item.description || item.lecture_title || "Homework details pending."}</p>
                </td>
                <td className="px-4 py-4 text-[#245C4F]">{item.subject_name || "Not available"}</td>
                <td className="px-4 py-4 text-[#245C4F]">{item.teacher_name || "Not available"}</td>
                <td className="px-4 py-4 text-[#245C4F]">{formatDate(item.due_date || item.created_at)}</td>
            ...
            {!items.length ? (
              <tr>
                <td colSpan={6} className="px-4 py-6 text-sm text-[#245C4F]">No homework assigned.</td>
              </tr>
            ) : null}
          </tbody>
        </table>
      ...
      {activeItem ? (
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-[#063F32]/45 px-4 pt-28 pb-8">
          <div className="w-full max-w-2xl overflow-hidden rounded-[2rem] border border-[#2D8A6A]/15 bg-white shadow-[0_24px_80px_-36px_rgba(13,59,46,0.24)]">
