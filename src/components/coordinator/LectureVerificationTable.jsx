    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative space-y-4">
        ...
        items.map((item) => (
          <article key={item.id} className="rounded-[1.75rem] border border-[#2D8A6A]/15 bg-[linear-gradient(180deg,rgba(255,255,255,0.96)_0%,rgba(250,247,240,0.98)_100%)] p-5 shadow-[0_20px_70px_-36px_rgba(6,63,50,0.18)] backdrop-blur-xl">
            {(() => {
              ...
              const displayStatus = String(item.display_status || getLectureDisplayStatus(item) || "").toLowerCase();
              ...
              const hideSyncButton =
                ["verified", "rejected", "verified_by_coordinator", "disputed"].includes(rawStatus) ||
                ["verified", "rejected"].includes(displayStatus) ||
                (hasTeacherAttendance && hasRecording);
      ...
      {rejectingItem ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#063F32]/45 px-4 py-8 backdrop-blur-sm">
          <div className="w-full max-w-2xl rounded-[2rem] border border-[#2D8A6A]/15 bg-[linear-gradient(180deg,rgba(255,255,255,0.98)_0%,rgba(250,247,240,0.99)_100%)] p-6 shadow-[0_30px_90px_-36px_rgba(13,59,46,0.28)] backdrop-blur-xl">
