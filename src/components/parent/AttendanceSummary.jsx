  ...
  return (
    <section className="rounded-[1.75rem] border border-[#2D8A6A]/15 bg-white/90 p-5 shadow-[0_20px_70px_-36px_rgba(13,59,46,0.18)]">
      ...
      <div className="space-y-3">
        {items.length ? (
          <>
            <div className="overflow-x-auto rounded-2xl border border-[#2D8A6A]/15 bg-[#FAF7F0]">
                ...
                <tbody className="divide-y divide-[#F1EADC]">
                  {visibleItems.map((item, index) => (
                    <tr key={`${item.id || "attendance"}-${index}`}>
                      <td className="px-4 py-4 font-semibold text-[#063F32]">{item.class_title || "-"}</td>
                      <td className="px-4 py-4 text-[#245C4F]">{item.subject_name || "Subject not set"}</td>
                      <td className="px-4 py-4 text-[#245C4F]">{formatDateTimeRange(item.scheduled_start, item.scheduled_end)}</td>
                      <td className="px-4 py-4 text-[#245C4F]">{item.status}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
