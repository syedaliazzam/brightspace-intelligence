...
export default function StudentDashboardPage() {
  ...
  useEffect(() => {
    const filters = { ...state.filters, date: todayDate() };
    setState((current) => ({ ...current, filters }));
    loadDashboard().catch((error) => setState((current) => ({ ...current, error: error.message })));
    loadHomework().catch((error) => setState((current) => ({ ...current, error: error.message })));
    loadAttendance().catch((error) => setState((current) => ({ ...current, error: error.message })));
    loadProfile().catch((error) => setState((current) => ({ ...current, error: error.message })));
    loadLectures(filters).catch((error) => setState((current) => ({ ...current, loading: false, error: error.message })));
  }, []);
