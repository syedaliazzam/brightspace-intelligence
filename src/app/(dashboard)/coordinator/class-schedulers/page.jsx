...
export default function CoordinatorClassSchedulersPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/coordinator/lecture-schedules?${params.toString()}`, {
        cache: "no-store",
      });
      const data = await response.json();

      if (!response.ok) {
        throw new Error(data?.message || "Unable to load class schedulars.");
      }
      ...
      setState({
        loading: false,
        error: error instanceof Error ? error.message : "Unable to load class schedulars.",
        items: [],
      });
    }
  ...
  return (
    <div className="min-h-screen bg-[#FAF7F0] text-[#063F32]">
      ...
      <div className="relative mx-auto max-w-7xl space-y-6 px-4 py-5 sm:px-6 lg:px-8">
          ...
          <div className="relative flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-5xl">
              ...
              <h1 className="mb-3 mt-4 text-3xl font-bold text-white-deep sm:text-4xl font-display">
                All Class Lectures Calendar
              </h1>
              <p className="mt-3 text-sm leading-7 text-[#EAF6EF] sm:text-base">
                View all class lectures in calendar format while keeping your lecture creation and verification pages separate.
              </p>
            </div>
        ...
        <LMSCalendar
          apiUrl="/api/coordinator/lecture-schedules"
