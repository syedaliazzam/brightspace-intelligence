...
export default function RegistrationPage() {
  ...
  function validate() {
    ...
    if (!form.studentAge.trim()) {
      nextErrors.studentAge = "Student age is required.";
    } else if (Number.isNaN(Number(form.studentAge)) || Number(form.studentAge) <= 0) {
      nextErrors.studentAge = "Enter a valid age.";
    }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setForm(initialForm);
      setSuccessMessage(
        "Registration submitted successfully. Our coordinator will contact you soon."
      );
    } catch (error) {
      setErrors((current) => ({
        ...current,
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#f5f7fb] text-slate-900">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-7xl items-center px-4 py-10 sm:px-6 lg:px-8">
        <motion.div
          className="grid w-full items-stretch gap-8 lg:grid-cols-[1.02fr_0.98fr]"
          variants={container}
          initial="hidden"
          animate="show"
        >
          ...
          >
            <div className="max-w-xl">
              ...
              <h1 className="mt-6 text-4xl font-semibold tracking-tight text-slate-950 sm:text-5xl">
                Start your child&apos;s LMS registration in one simple form.
              </h1>
              <p className="mt-5 max-w-lg text-base leading-7 text-slate-600 sm:text-lg">
                Share the basic student and parent details. Our coordinator will review the request and contact you for the next steps.
              </p>
            </div>

            <div className="mt-10 grid gap-4 sm:grid-cols-3">
              {[
                "Quick coordinator follow-up",
                "Clean registration workflow",
                "Secure direct LMS intake",
              ].map((label) => (
                <div
                  key={label}
            ...
            <div className="mt-10 rounded-3xl bg-slate-950 px-6 py-6 text-white shadow-lg">
              ...
              <p className="mt-3 max-w-md text-sm leading-6 text-slate-300">
                New submissions go directly into the coordinator registration lead queue so the admissions process can begin without external forms.
              </p>
            </div>
          </motion.section>
          ...
          >
            <div className="mb-8">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Complete the required fields and we&apos;ll route the request to the coordinator portal.
              </p>
            </div>
            ...
            <form className="grid gap-5 sm:grid-cols-2" onSubmit={handleSubmit}>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="parentName">
                  Parent Name
                </label>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="parentRelation">
                  Parent Relation
                </label>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="parentEmail">
                  Parent Email
                </label>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="studentAge">
                  Student Age
                </label>
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="classLevel">
                  Class Level
                </label>
                <select id="classLevel" value={form.classLevel} onChange={(event) => updateField("classLevel", event.target.value)} className={inputClass(errors.classLevel)}>
                  ...
                  {[...ALLOWED_CLASS_LEVELS].map((option) => (
                    <option key={option} value={option}>
                      {option}
                    </option>
                  ))}
              ...
              <div>
                <label className="mb-2 block text-sm font-medium text-slate-700" htmlFor="preferredSchedule">
                  Preferred Schedule
                </label>
                <select id="preferredSchedule" value={form.preferredSchedule} onChange={(event) => updateField("preferredSchedule", event.target.value)} className={inputClass(false)}>
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.99 }}
                disabled={pending}
                className="inline-flex w-full items-center justify-center rounded-2xl bg-slate-950 px-4 py-3.5 text-sm font-semibold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-70 sm:col-span-2"
              >
