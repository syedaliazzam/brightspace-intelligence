...
export async function POST(request) {
  try {
    ...
    const documentType = sanitizeFilename(body?.documentType || "library");
    ...
    const objectPath = `library-documents/${documentType}/${Date.now()}_${fileName}`;
    ...
    return json("Library document upload URL created.", 200, {
      signedUrl,
      token,
